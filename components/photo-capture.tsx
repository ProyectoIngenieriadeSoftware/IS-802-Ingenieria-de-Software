...
export function PhotoCapture({ onPhotoCapture, existingPhoto }: PhotoCaptureProps) {
  ...
  useEffect(() => {
    return () => {
      console.log("[v0] PhotoCapture unmounting, stopping camera")
  ...
  const startCamera = useCallback(async () => {
    console.log("[v0] Iniciando cámara...")
    ...
    try {
      ...
      console.log("[v0] Stream obtenido:", stream)
      ...
      if (videoRef.current) {
        ...
        console.log("[v0] Video reproduciendo")
  ...
  const stopCamera = useCallback(() => {
    console.log("[v0] Deteniendo cámara...")
    if (streamRef.current) {
      streamRef.current.getTracks().forEach((track) => track.stop()) // Detener los tracks de la cámara
