...
import { ArrowLeft, User, GraduationCap, Briefcase, AlertCircle } from "lucide-react"
...
interface UserDashboardProps {
  ...
  onProceedToEntry: () => void // Función para proceder con el registro de entrada
}
        ...
        <Card className="w-full max-w-2xl p-8 md:p-12 bg-white shadow-lg border border-border">
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              {/* Mostrar DNI del usuario */}
              <div className="flex items-center gap-2">
                <span className="text-gray-600">DNI:</span>
                <span className="font-mono font-medium text-[#003876]">{dni}</span>
              </div>
            ...
            {/* Botón para continuar con el registro de ingreso */}
            <div className="pt-4">
              <Button
                onClick={onProceedToEntry}
                disabled={!isActive} // Deshabilitar si la cuenta está inactiva
                className="w-full h-12 text-base font-bold bg-[#FFC107] hover:bg-[#FFB300] text-[#003876] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isActive ? "Continuar con registro de ingreso" : "Cuenta inactiva - No se puede ingresar"}
              </Button>
            </div>
          </div>
        </Card>
